...
create policy "Public can update appointment statuses"
on public.repple_appointments
for update
using (generated_id is not null or legacy_generated_id is not null)
with check (generated_id is not null or legacy_generated_id is not null);

grant select, insert, update on public.repple_appointments to anon, authenticated;
...
grant select, insert on public.repple_vehicle_image_cache to anon, authenticated;
